...
function cityInfoSetUp(%name, %background, %coords, %button, %altCoords, %altButton) {
    ...
    "VenuesMap".bindClassName(%cityInfo.venues);
    return %cityInfo;
};
function cityInfoAddVenue(%cityInfo, %venueName, %coords, %button, %spawnPointsGroup) {
    ...
    %venueInfo.put(%cityInfo.venues, %venueInfo.name);
    if ($StandAlone) {
    }
...
function fillCityInfoMap(%map) {
    ...
    %cityInfo.put(%map, %cityInfo.name);
...
function fillCityNamesMap(%map) {
    ...
    %code = "nv";
    %long = "NewVenezia";
    %code.put(%map, "New Venezia");
    %code.put(%map, %long);
    %code.put(%map, %code);
    %long.put(gCityNamesShortToLongMap, %code);
    %code = "lga";
    %long = "LaGenoaAires";
    %code.put(%map, "La Genoa Aires");
    %code.put(%map, %long);
    %code.put(%map, %code);
    %long.put(gCityNamesShortToLongMap, %code);
    %code = "rj";
    %long = "RaiJuku";
    %code.put(%map, "Rai Juku");
    %code.put(%map, %long);
    %code.put(%map, %code);
    %long.put(gCityNamesShortToLongMap, %code);
    %code = "gw";
    %long = "Gateway";
    %code.put(%map, "n00bieIsland");
    %code.put(%map, %long);
    %code.put(%map, %code);
    %long.put(gCityNamesShortToLongMap, %code);
    return %map;
