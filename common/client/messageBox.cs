...
function MessageBoxOK(%title, %message, %callback, %canStopShowing, %key) {
    ...
    %dialog.callback = Canvas @ %callback @ 0;
    ModalMessageBoxes;
    return %dialog;
};
function MessageBoxTextEntry(%title, %message, %callback, %defaultText) {
    ...
    %dialog.pushDialog(0);
    %dialog.setMessageText(%message);
    %dialog.callback = Canvas @ %callback;
    ModalMessageBoxes;
...
function MessageBoxTextEntryWithBitmapWithCancel(%title, %message, %callback, %defaultText, %maxLength, %bitmapPath) {
    ...
    0;
    %ctrl = new ""() {
        position = GuiBitmapCtrl @ %posX @ " " @ %posY;
        extent = %extX @ " " @ %extY;
        bitmap = %bitmapPath;
    };
    %container.add(%ctrl);
...
function MessageBoxYesNo(%title, %message, %yesCallback, %noCallback, %canStopShowing, %key) {
    ...
    %dialog.pushDialog(0);
    %dialog.setMessageText(%message);
    %dialog.callback = Canvas @ %yesCallback @ 0;
    ModalMessageBoxes;
...
function MessageBox::newDialog(%buttonList) {
    ...
    0;
    %window = new ""() {
        profile = GuiWindowCtrl @ "GuiMessageWindowProfile";
        horizSizing = "center";
        vertSizing = "center";
        position = "170 175";
...
function MessageBox::tryAddStopShowing(%this, %title, %message, %canStopShowing, %key) {
    if (!(%canStopShowing $= "")) {
        ...
        0;
        %ctrl = new ""() {
            profile = GuiCheckBoxCtrl @ ETSLoginSmallCheckBoxProfile;
