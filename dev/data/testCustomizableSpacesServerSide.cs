...
function TEST_CS_DoubleActivateSpace::runTest(%this) {
    ...
    lastFreeNumber = %freeGridNumber @ %this;
    %client = 0.getObject();
    ClientGroup;
    %userName = nameBase;
    %client;
    %player = Player;
    %client;
    %apartmentName = MissionInfo @ GetServerNameSpaceTaggedName(building) @ "." @ %userName;
...
function TEST_CS_DoubleActivateSpace::delayedEval(%this) {
    ...
    %freeGridNumber = CustomizableSpaceServerGrid::GetNextUnActivatedSpaceGridNumber();
    %this.assert((%this != lastFreeNumber), "I expected to have taken that grid space, but it is still marked as free");
};
function TEST_CS_DoubleActivateSpaceType2::runTest(%this) {
    ...
    %player = Player;
    %client;
    %apartmentName = MissionInfo @ MissionInfo @ GetServerNameSpaceTaggedName(building) @ "." @ %userName;
    %freeGridNumber = CustomizableSpaceServerGrid::GetNextUnActivatedSpaceGridNumber();
    %this.assert((0.0 > %freeGridNumber), "I couldn't find any free grid space to run this test with");
    lastFreeNumber = %freeGridNumber @ %this;
    %theSpaceTrigger = "PRIVATESPACE_AREA_" @ %freeGridNumber;
