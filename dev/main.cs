...
package dev {
    ...
    function onDoneOrErrorCallback_generateRegistration(%request) {
        ...
        if (%f.openForAppend("platform/client/default_owner.cs")) {
            ...
            %f.writeLine(0 @ "$Net::registrationID = \"" @ %registrationID @ "\";");
        }
        %f.close();
        quit();
    };
    function compileScripts(%extensions) {
        ...
        if ((getWordCount(%extensions) < %n)) {
            ...
            if (!(%file $= "")) {
                ...
                if (!(%suc)) {
                    %tryCount[%fails @ (%sucCount - %tryCount)] = %file;
                }
